...
;(function () {
  ...
  function applyFallbackBadge(badge) {
    ...
    badge.setAttribute('title', t('fallbackBadgeTitle'))

    const label = document.createElement('span')
    label.textContent = t('setUpProfile')
    label.style.cssText = 'font-weight:600;'
  ...
  function buildPopoverContent(score, data) {
    ...
    const tier = fallback
      ? { bg: '#f4f4f5', dot: '#a1a1aa', fg: '#71717a', label: t('tierProfileIncomplete') }
      : getMatchTier(score)
    ...
    titleText.innerHTML = fallback
      ? `<div class="jobswiper-axis-title-main">${escHtml(t('completeYourProfile'))}</div>
      <div class="jobswiper-axis-title-sub"><span style="color:${tier.fg}">${escHtml(tier.label)}</span></div>`
      : `<div class="jobswiper-axis-title-main">${escHtml(t('whyThisScore'))}</div>
