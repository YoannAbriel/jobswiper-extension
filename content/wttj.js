...
async function handleSave(btn, jobDataOverride, retryCount = 0) {
  ...
  if (!jobData.title || !jobData.company) {
    ...
    const aiJob = await window.JobSwiperCapture.aiExtractFallback({ source: 'wttj' })
    if (!aiJob) {
...
function injectDetailButton() {
  ...
  // Fetch score (through the SW so a refreshed token is used). Capture the
  // button this fetch belongs to: if the badge is gone when the response lands
  // (navigated to another job), do NOT mislabel the new job "Saved" (#5).
  ...
  ;(async () => {
    ...
    try {
      ...
      if (!scoreBadge.isConnected) return // navigated to another job mid-flight
