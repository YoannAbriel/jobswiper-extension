...
function createSaveButton() {
  ...
  btn.innerHTML = `<div class="jobswiper-beam"></div>${_logoUrl ? `<span class="jobswiper-logo-wrap"><img src="${_logoUrl}" width="14" height="14"></span> ` : ''}Save to JobSwiper`
  return btn
}
...
async function handleSave(btn, retryCount = 0) {
  ...
  try {
    ...
    if (response && response.success) {
      ...
      btn.innerHTML = `<div class="jobswiper-beam"></div>${_logoUrl ? `<span class="jobswiper-logo-wrap"><img src="${_logoUrl}" width="14" height="14"></span> ` : ''}Saved!`
